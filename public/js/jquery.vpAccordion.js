...
;( function( $, window, undefined ) {
	...
	$.CBPNTAccordion.prototype = {
		...
		_config : function() {
			...
			// the clickable items
			this.$items = this.$el.find( '.ac-trigger' );
		...
		_initEvents : function() {
			...
			this.$items.on( 'click.cbpNTAccordion', function() {
				...
				if( $listItem.hasClass( 'ac-open' ) ) {
					$listItem.removeClass( 'ac-open' );
				}
				else {
					$listItem.addClass( 'ac-open' );
		...
		destroy : function() {
			this.$items.off( '.cbpNTAccordion' ).parent().removeClass( 'ac-open' );
		}
	};
